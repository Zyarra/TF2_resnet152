...
def get_images_and_labels(data_root_dir):
    ...
    # get labels' names
    label_names = sorted(set(item.parent.name for item in data_root.glob('*/*')))
    # dict: {label : index}
    label_to_index = dict((label, index) for index, label in enumerate(label_names))
    # get all images' labels
    all_image_label = [label_to_index[pathlib.Path(single_image_path).parent.name] for single_image_path in all_image_path]
    # print(all_image_path, all_image_label)
...
def generate_datasets():
    ...
    valid_dataset = valid_dataset.shuffle(buffer_size=valid_count).batch(batch_size=config.BATCH_SIZE)
    test_dataset = test_dataset.shuffle(buffer_size=test_count).batch(batch_size=config.BATCH_SIZE)
